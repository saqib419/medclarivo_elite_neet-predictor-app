"""
Merge official NMC total-seat data into data.json — v7.
...
1. Duplicate-row vs genuine-conflict split.
   ...
   to v7_nmc_duplicate_rows.txt (please verify/merge these rows later
   ...
   like v5 did, logged to v7_nmc_duplicate_targets.txt.

2. Distinguishing-suffix guard (fixes a v6 false positive).
   v6's dry-run incorrectly classified "Malla Reddy Institute of
   Medical Sciences" and "Malla Reddy Medical College for Women" --
   two real, different colleges -- as duplicate rows of the same
   college, because their text similarity (~0.62) squeaked past the
   0.60 threshold on shared "Malla Reddy" wording. v7 adds
   DISTINGUISHING_SUFFIX_WORDS (women, men, boys, girls, campus,
   north/south/east/west, etc.): if such a word appears in some but
   not all of a colliding group's app names, the group is forced to
   genuine-conflict regardless of similarity score. Location-only
   differences (e.g. Zoram Medical College, Falkawn vs ...,
   Mizoram) are unaffected and still correctly classify as
   duplicate rows.

3. Missing-college check.
   ...
   are written to v7_nmc_missing_from_app.json (name, state,
...
Usage:
    python3 merge_nmc_totals_v7.py --dry-run
    python3 merge_nmc_totals_v7.py --apply
"""
...
# Words that mark a genuinely distinct institution/campus rather than
# just an address/location detail. If a word from this set appears in
# ONE app name in a colliding group but not the other(s), that group
# is forced to genuine-conflict regardless of similarity score --
# this is what v7 got wrong for "Malla Reddy Institute of Medical
# Sciences" vs "Malla Reddy Medical College for Women": high text
# similarity (shared "Malla Reddy"), but "for Women" marks a real,
# separate institution, not a location suffix like Falkawn/Mizoram.
...
def has_distinguishing_mismatch(app_names):
    """True if a distinguishing-suffix word (women, campus, north,
    etc.) appears in some but not all of the app names in the group --
    a sign these are genuinely different institutions, not the same
    college listed twice with location variants."""
    per_name_words = []
    for n in app_names:
...
def group_is_duplicate_rows(app_names):
    ...
    of names in it is highly similar to every other pair AND none of
    them carry a mismatched distinguishing-suffix word (see
    DISTINGUISHING_SUFFIX_WORDS). A single dissimilar pair, or a
    mismatched distinguishing word, means it's a genuine conflict
    instead."""
    if len(app_names) < 2:
        return True
...
def main():
    ...
    with open("v7_nmc_unmatched.txt", "w") as f:
        f.write("\n".join(unmatched))
    with open("v7_nmc_changes.json", "w") as f:
        json.dump(changes, f, indent=2, default=str)
    with open("v7_nmc_duplicate_targets.txt", "w") as f:
        for official_name in genuine_conflict_targets:
            f.write(f"Official record: {official_name}\n")
            for an in target_to_apps[official_name]:
                f.write(f"  <- {an}\n")
            f.write("\n")
    with open("v7_nmc_duplicate_rows.txt", "w") as f:
        for official_name in duplicate_row_targets:
            f.write(f"Official record: {official_name}\n")
            for an in target_to_apps[official_name]:
                f.write(f"  <- {an}\n")
            f.write("\n")
    with open("v7_nmc_skipped_ini.txt", "w") as f:
        f.write("\n".join(skipped_ini))
    with open("v7_nmc_missing_from_app.json", "w") as f:
        json.dump(missing, f, indent=2, default=str)

    print(f"\nWrote v7_nmc_unmatched.txt, v7_nmc_changes.json, v7_nmc_duplicate_targets.txt,")
    print(f"v7_nmc_duplicate_rows.txt, v7_nmc_skipped_ini.txt, v7_nmc_missing_from_app.json")
